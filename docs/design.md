...
### 9.6 To verify in M0 (the docs are silent)

WebSocket frames count as idle-policy traffic (else HTTP pings) · an open socket survives its token's expiry (else 60-minute tokens, reconnect at 55) · endpoint 429 thresholds (else coalesce events, lower heartbeat rate) · DNS and S3 reachability inside the image via a diagnostics route (else a VPC egress connector) · the number of concurrent WebSocket connections one endpoint accepts (else a lower node cap) · per-connection message rate limits (else batching) · resume latency at 1 GB (else smaller baseline or a scheduled warm ping) · CreateMicrovmAuthToken throttling (informational, since tokens are shared) · that a rotation under load keeps the session function's concurrent invocations under 10 — the drain jitter is the mechanism that makes the design work at the account default, measured at M3; the increase to 1000 requested on 2026-09-01 stays as extra margin, not as the fix. The memory quota (8 GB) and RunMicrovm rate (1/s) are already known.
