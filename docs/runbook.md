...
| The dashboard shows "The machine is asleep" for a few seconds at the start of a rotation | seen once with dashboards connected (WP4.4); heals on the next snapshot; cause not yet pinned — suspects: the sleep clock across the handover, a successor reused from an interrupted rotation | `node packages/infra/scripts/rotation-probe.ts` watches a rotation the way a browser does |
