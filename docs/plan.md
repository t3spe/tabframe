...
- [ ] **WP4.9 Seeding follow-through and snapshot diet.** Found by the first verify-m1 on the paced image: a deploy that changes a program seeds a second `mandelbrot` (a new bundle hash) but leaves the default loop on the old bundle and both in the program list, so the machine kept rendering the old frame. Seeding now moves the default loop to the shipped bundle and retires the superseded seeded record (executions keep their copies). The ended executions' file maps — 21 000 entries at 32 frames — are what keeps the deployed snapshot near a megabyte: retired with the tasks beyond the two most recent frames, and rebuilt from the root manifest when a launch inherits an older execution. Runbook: the memory samples (RSS 312 → 372 MiB over 33 min, flat) and the duplicate-program symptom. Deploy, rotate, `mise run verify:m1` green.
