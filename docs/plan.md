...
- [x] **WP7.2 The throughput chart and the small rows.** _Done 2026-09-03 (`docs/implementation/wp-7.2-throughput-rows.md`)._ D2: the chart is one crisp area line at the device pixel ratio on a round scale that moves at most once a minute, with a caption; counters in two rows of four; the redundancy toggle on its own line with a tooltip; the flashes as one line of text under the legend.
- [x] **WP7.3 The launch form keeps focus.** _Done 2026-09-03 (`docs/implementation/wp-7.3-launch-form-focus.md`)._ One DOM row per program and one launch form per open program, updated in place; rows move only when the order changes; params parsed on blur and on launch with the error under the box.
- [x] **WP7.4 Files and ledger rows open in the page.** _Done 2026-09-03 (`docs/implementation/wp-7.4-files-ledger-in-page.md`)._ D3: one preview box per tab, always there, fed by a file click or a ledger click (bars, text, tile, manifest, hex); a click toggles a class, the lists are not rebuilt; "↗" and "raw ↗" are the small secondary links. **Deploy 1: generation 99, 2026-09-04; three unattended demo passes green.**
