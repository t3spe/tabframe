...
5. **Observability:** MicroVM logs never reached CloudWatch during the build; the machine is read
   through its own sockets and S3, and that gap is tracked.

## How long it took

The assignment grades scoping against an eight-hour ceiling and asks for the time spent. This
build took many days by deliberate choice — the design walkthrough alone was a full day, every
milestone was verified against AWS, and I kept a time log with two columns because the honest
number has two parts. `docs/timelog.md` records **<<total hours>>** of wall-clock session time in
total (20.5 hours across the three sessions logged when this draft was written; the later sessions
are still to be entered), of which **<<developer hours>>** were my own hours at the keyboard.
